...
const Auth: React.FC<AuthProps> = ({ onNavigate, onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // onNavigate(Page.HOME); // App.tsx will handle navigation via onLogin
      } else {
        // Đăng ký
        ...
        <div className="p-8 pt-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-12 bg-primary hover:bg-primary-dark text-white rounded-xl text-sm font-black tracking-widest uppercase transition-all shadow-xl shadow-primary/20 flex items-center justify-center gap-3 active:scale-[0.98] group disabled:opacity-50 disabled:cursor-not-allowed"
            >
