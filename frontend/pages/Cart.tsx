    ...
    <div className="flex flex-col gap-10 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-wrap justify-between items-end gap-4 border-b border-slate-100 dark:border-surface-border pb-8">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-light">Chọn các sản phẩm bạn muốn thanh toán.</p>
        </div>
        <button
          onClick={onClearCart}
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="bg-slate-50 dark:bg-black/20 border-b border-slate-100 dark:border-surface-border">
                  ...
                  <th className="px-4 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 w-1/2">Sản phẩm</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-surface-border">
                {cart.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className={`group transition-colors ${isSelected ? 'bg-primary/[0.03] hover:bg-primary/[0.06]' : 'hover:bg-slate-50 dark:hover:bg-white/5'}`}>
            ...
            <div className="rounded-3xl border border-slate-100 dark:border-surface-border bg-white dark:bg-surface-dark p-8 shadow-2xl shadow-slate-200/50 dark:shadow-none relative overflow-hidden">
              ...
              {selectedItems.length === 0 ? (
                <div className="text-center py-6">
                  <span className="material-symbols-outlined text-slate-300 !text-[40px] mb-2">check_box_outline_blank</span>
                  <p className="text-sm text-slate-400 font-medium">Chưa chọn sản phẩm nào</p>
                  <p className="text-xs text-slate-400 mt-1">Hãy chọn sản phẩm bạn muốn thanh toán</p>
                </div>
              ) : (
                <>
              ...
              >
                ...
                {selectedItems.length > 0 && <span className="material-symbols-outlined group-hover:translate-x-1 transition-transform">arrow_forward</span>}
              </button>
            </div>
